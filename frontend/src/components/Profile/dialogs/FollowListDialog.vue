...
async function loadPage() {
  ...
  const result = await api.followingPage(props.targetUserId, page.value, pageSize)
  ...
  <Teleport to="body">
    <Transition name="relation-dialog">
      <div v-if="visible" class="relation-dialog__backdrop" @click.self="emit('close')">
        <section class="relation-dialog__panel" role="dialog" aria-modal="true" aria-label="关注列表">
          <header class="relation-dialog__header">
            <div class="relation-dialog__tabs">
              <button type="button" class="is-active" @click="emit('switchTab', 'follow')">关注 ({{ total }})</button>
              <button type="button" @click="emit('switchTab', 'fans')">粉丝 ({{ fansTotal }})</button>
            </div>
          ...
          <ul class="relation-dialog__list">
            <li v-for="user in filteredList" :key="user.id" class="relation-dialog__item" @click="emit('openUser', user.id)">
              ...
              <button type="button" @click.stop>相互关注</button>
          ...
          <p v-else-if="!infinite.loading.value && !filteredList.length" class="relation-dialog__state">暂无关注</p>
        </section>
      </div>
    </Transition>
  </Teleport>
...
.relation-dialog__item button:not(.is-muted) {
  background: #f6f7f9;
}
